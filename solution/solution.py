...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        The retrieval metrics are intentionally excluded: they diagnose the
        retriever, while this score measures answer quality.
        """
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "Return a JSON object with numeric scores from 0 to 1, using the "
            "rubric criterion names as keys. You may include a reasoning field.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
        ...
        # Accept plain JSON, fenced JSON, or a JSON object embedded in a short
        # explanation. This keeps the callable easy to mock while remaining
        # tolerant of common judge-model formatting.
        candidates = [raw_text]
        fenced = re.search(r"```(?:json)?\s*(\{.*?\})\s*```", raw_text, re.DOTALL)
        if fenced:
            candidates.insert(0, fenced.group(1))
        embedded = re.search(r"\{.*\}", raw_text, re.DOTALL)
        if embedded:
        ...
        # Missing or malformed scores use the documented neutral fallback.
        scores = {criterion: scores.get(criterion, 0.5) for criterion in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Positional bias requires position-labelled comparisons. The normal
        # score_response result has no ordering metadata, so do not infer this
        # bias from criterion order. Support common batch annotations used by
        # tests/experiments: first_score/second_score or position_scores.
        position_pairs: list[tuple[float, float]] = []
        for result in scores_batch:
            pair = result.get("position_scores")
            if isinstance(pair, (list, tuple)) and len(pair) >= 2:
                if all(isinstance(value, (int, float)) for value in pair[:2]):
                    position_pairs.append((float(pair[0]), float(pair[1])))
            elif isinstance(pair, dict):
                first = pair.get("first", pair.get("position_1"))
                second = pair.get("second", pair.get("position_2"))
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # The evaluator computes scores; the benchmark result must still
            # point to the original golden-dataset pair and its metadata.
            result.qa_pair = pair
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        low_metrics = [name for name, score in scores.items() if score < 0.5]
        if len(low_metrics) > 1:
            return "Multiple issues detected — review full pipeline"

        lowest_metric = min(scores, key=scores.get)
        if lowest_metric in causes:
            return causes[lowest_metric]
        return "Multiple issues detected — review full pipeline"
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        default_suggestion = "Review the failure and add a regression test."
        for index, failure in enumerate(failures, start=1):
            ...
            # Keep the table valid when model-generated text contains pipes.
            values = [failure_type, root_cause, suggestion]
            values = [str(value).replace("|", "\\|") for value in values]
            lines.append(
                f"| F{index:03d} | {values[0]} | {values[1]} | "
                f"{values[2]} | Open |"
            )
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": (
                "Add a grounding check and reject claims that are unsupported by retrieved context."
            ),
            "irrelevant": (
                "Clarify intent-routing and prompt instructions so the answer directly addresses the question."
            ),
            "incomplete": (
                "Increase useful context coverage and add few-shot examples requiring every expected point."
            ),
            "off_topic": (
                "Add intent-detection tests and an explicit scope check before generation."
            ),
            "refusal": (
                "Review guardrails and add in-scope examples to reduce unnecessary refusals."
            ),
            "unknown": (
                "Inspect the failing traces and add a regression test for the unclassified failure."
            ),
        }

        # Preserve a useful priority order rather than depending on input order.
        for failure_type in (
            "hallucination",
            "irrelevant",
            "incomplete",
            ...
            if failure_type in categories:
                suggestions.append(suggestions_by_type[failure_type])

        # A single failure can expose several remediation needs. Keep the
        # output actionable and satisfy the lab's minimum recommendation set.
        fallback_suggestions = [
            "Add representative golden-set cases and run them in CI on every prompt or model change.",
            "Calibrate evaluation thresholds against human labels and review borderline cases.",
        ]
